...
public class NetflixRecommender {
    ...
    public NetflixRecommender(Reader reader) {
        ...
        try(BufferedReader br = new BufferedReader(reader)) {
            content = br.lines().map(Content::of).toList();
        }
        catch (IOException e) {
    ...
    public Map<ContentType, Set<Content>> groupContentByType() {
        ...
        return content.stream().reduce(res, (map, c) -> add(map,c))
    }
    ...
    public List<Content> getTopNRatedContent(int n) {
       return content.stream().sorted(new ContentComperator()).limit(n).toList();
    }
    ...
    public List<Content> getSimilarContent(Content content) {
        return this.content.stream().filter(c -> c.type() == content.type()).
    }
